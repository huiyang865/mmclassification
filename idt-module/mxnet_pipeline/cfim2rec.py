...
def read_list(path_in):
    with open(path_in) as fin:
        while True:
            ...
            line_len = len(line)
            if line_len < 3:
                print('lst should at least has three parts, \
...
def image_encode(args, i, item, q_out):
    ...
    try:
        # if '/apps/' not in fullpath:
        # fullpath = '/apps/'+fullpath
...
def fillImg(img):
    ...
    rimg = cv2.resize(fimg, (224, 224), interpolation=cv2.INTER_AREA)
    return rimg
...
def parse_args():
    ...
    cgroup.add_argument(
        ...
        by traversing root folder \
        and output to <prefix>.lst.\
        Otherwise im2rec will read <prefix>.\
        lst and create a database at <prefix>.rec')
    cgroup.add_argument(
        '--exts',
        nargs='+',
    ...
    cgroup.add_argument(
        '--recursive',
        action='store_true',
        help='If true recursively walk through subdirs \
        and assign an unique label\
        to images in each folder. \
        Otherwise only include images in the root folder\
    ...
    rgroup.add_argument(
        ...
        original images will\
        be packed by default.')
    rgroup.add_argument(
        '--center-crop',
        action='store_true',
        help='specify whether to crop the center \
        image to make it rectangular.')
    rgroup.add_argument(
        '--quality',
        type=int,
        default=95,
    ...
    rgroup.add_argument(
        ...
        order of images will be different\
        from the input list if >1. \
        the input list will be modified to match the\
        resulting order.')
    rgroup.add_argument(
        '--color',
        type=int,
    ...
    rgroup.add_argument(
        ...
        action='store_true',
        default=True,
        help='Whether to also pack multi \
        dimensional label in the record file')
