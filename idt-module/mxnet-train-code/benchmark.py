...
def setup_logging(log_loc):
    if os.path.exists(log_loc):
        shutil.move(log_loc,
                    log_loc + '_' + str(int(os.path.getctime(log_loc))))
    os.makedirs(log_loc)
...
class RunCmd(threading.Thread):
    ...
    def startCmd(self, timeout):
        LOGGER.debug('Attempting to start Thread to run %s',
                     ' '.join(self.cmd_args))
        self.start()
        self.join(timeout)
        if self.is_alive():
            LOGGER.debug('Terminating process running %s',
                         ' '.join(self.cmd_args))
...
def parse_args():
    ...
    class NetworkArgumentAction(argparse.Action):
        ...
        def validate(self, attrs):
            args = attrs.split(':')
            if len(args) != 4 or isinstance(args[0],
                                            str) is False or isinstance(
                                                args[1], str) is False:
                print('expected network attributes in format mode:\
                    network_name:batch_size:image_size \
                \nThe network_name is a valid model defined as \
                network_name.py in the image-classification/symbol folder. \
    ...
    parser = argparse.ArgumentParser(
        description='Run Benchmark on various imagenet \
        networks using train_imagenent.py')
    parser.add_argument(
        '--networks',
        dest='networks',
        nargs='+',
        type=str,
        help='one or more networks in the format mode:\
        network_name:batch_size:image_size \
    \nThe network_name is a valid model defined as network_name.py \
    in the image-classification/symbol folder for native imagenet \
    \n Or a gluon vision model defined in \
    mxnet/python/mxnet/gluon/model_zoo/model_store.py.',
    ...
    parser.add_argument(
        '--worker_file',
        type=str,
        help='file that contains a list of worker hostnames \
        or list of worker \
        ip addresses that can be sshed without a password.',
        required=True)
    parser.add_argument(
        '--worker_count',
        type=int,
    ...
    parser.add_argument(
        '--gpu_count',
        type=int,
        help='number of gpus on each worker to use.',
        required=True)
...
def images_processed(log_loc, mode):
    f = open(log_loc)
    if mode == 'native':
        img_per_sec = re.findall(
            r'(?:Batch\s+\[30\]\\\\tSpeed:\s+)(\d+\.\d+)(?:\s+)',
            str(f.readlines()))
    else:
        img_per_sec = re.findall(
            r'(?:Batch\s+\[3\]\\\\tSpeed:\s+)(\d+\.\d+)(?:\s+)',
            str(f.readlines()))
    f.close()
...
def generate_hosts_file(num_nodes, workers_file, args_workers_file):
    ...
    output = subprocess.check_output(
        ['head', '-n', str(num_nodes), args_workers_file])
    f.write(output)
    f.close()
    return
...
def stop_old_processes(hosts_file, prog_name):
    stop_args = [
        'python', '../../tools/kill-mxnet.py', hosts_file, 'python', prog_name
    ]
...
def run_benchmark(kv_store, data_shape, batch_size, num_gpus, num_nodes,
    ...
    if mode == 'native':
        benchmark_args = [
            'python', 'train_imagenet.py', '--gpus',
            ','.join(str(i) for i in range(num_gpus)), '--network', network,
            '--batch-size',
            str(batch_size * num_gpus), '--image-shape',
            '3,' + str(data_shape) + ',' + str(data_shape), '--num-epochs',
            '1', '--kv-store', kv_store, '--benchmark', '1', '--disp-batches',
            '10'
        ]
    else:
        benchmark_args = [
            'python', '../gluon/image_classification.py', '--dataset', 'dummy',
            '--gpus',
            str(num_gpus), '--epochs', '1', '--benchmark', '--mode', mode,
            '--model', network, '--batch-size',
            str(batch_size), '--log-interval',
            str(1), '--kvstore', kv_store
        ]

    log = log_loc + '/' + network + '_' + str(num_nodes * num_gpus) + '_log'
    hosts = log_loc + '/' + network + '_' + str(
        num_nodes * num_gpus) + '_workers'
    ...
    launch_args = [
        '../../tools/launch.py', '-n',
        str(num_nodes), '-s',
        str(num_nodes * 2), '-H', hosts, ' '.join(benchmark_args)
    ]
...
def plot_graph(args):
    speedup_chart = pygal.Line(
        x_title='gpus', y_title='speedup', logarithmic=True)
    speedup_chart.x_labels = map(str,
                                 series(args.worker_count * args.gpu_count))
    speedup_chart.add('ideal speedup',
                      series(args.worker_count * args.gpu_count))
    for net in args.networks:
        image_single_gpu = net.gpu_speedup[
            1] if 1 in net.gpu_speedup or not net.gpu_speedup[1] else 1
        y_values = [
            each / image_single_gpu for each in net.gpu_speedup.values()
        ]
        LOGGER.info('%s: image_single_gpu:%.2f' % (net.name, image_single_gpu))
        LOGGER.debug('network:%s, y_values: %s' %
                     (net.name, ' '.join(map(str, y_values))))
        speedup_chart.add(
            net.name,
            y_values,
            formatter=lambda y_val, img=copy.deepcopy(
                image_single_gpu), batch_size=copy.deepcopy(net.batch_size):
            'speedup:%.2f, img/sec:%.2f, batch/gpu:%d' %
            (0 if y_val is None else y_val, 0
