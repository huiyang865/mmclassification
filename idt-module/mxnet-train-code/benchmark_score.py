...
def score(network, dev, batch_size, num_batches, dtype):
    ...
    mod.bind(
        for_training=False, inputs_need_grad=False, data_shapes=data_shape)
    mod.init_params(initializer=mx.init.Xavier(magnitude=2.))

    # get data
    data = [
        mx.random.uniform(-1.0, 1.0, shape=shape, ctx=dev)
...
if __name__ == '__main__':
    networks = [
        'alexnet', 'vgg-16', 'inception-bn', 'inception-v3', 'resnet-50',
        'resnet-152'
    ]
            ...
            for b in batch_sizes:
                for dtype in ['float32', 'float16']:
                    if d == mx.cpu() and dtype == 'float16':
                        ...
                        continue
                    elif net in ['inception-bn', 'alexnet'
                                 ] and dtype == 'float16':
                        if not logged_fp16_warning:
                            logging.info('Model definition for {} does \
                                not support float16'.format(net))
                        ...
                        speed = score(
                            ...
                            dtype=dtype)
                        logging.info(
                            'batch size %2d, dtype %s, images/sec: %f', b,
                            dtype, speed)
