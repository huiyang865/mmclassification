...
from .evaluate_core import ClassificationEvaluate, to_onehot
...
def _get_lr_scheduler(args, kv):
    ...
    step_epochs = [int(il) for il in args.lr_step_epochs.split(',')]
    lr = args.lr
    ...
    logging.info('learning_rate: ' + str(lr))
    if steps:
        return (lr,
...
def add_fit_args(parser):
    ...
    train.add_argument(
        '--save-period', type=int, default=1, help='params saving period')
    parser.add_argument(
        '--monitor',
        dest='monitor',
        type=int,
    ...
    train.add_argument(
        '--profile-worker-suffix',
        type=str,
        default='',
        help='profile workers actions into this file. \
        During distributed training \
                             filename saved will be rank1_ \
                             followed by this suffix')
    train.add_argument(
        '--profile-server-suffix',
        type=str,
        default='',
        help='profile server actions into a file with name \
                like rank1_ followed by this suffix \
                during distributed training')
    return train
...
def validate_model(model,
    ...
    """Runs prediction on ``eval_data`` and evaluates the performance according to
        ...
        <http://mxnet.io/tutorials/basic/module.html>`_ to see
        ...
        num_batch : int
            Number of batches to run. Defaults to ``None``,
        ...
        reset : bool
            Defaults to ``True``.
            Indicates whether we should reset `eval_data` before starting
            evaluating.
        epoch : int
            Defaults to 0. For compatibility,
            this will be passed to callbacks (if any).
            During training,
            this will correspond to the training epoch number.
        sparse_row_id_fn : A callback function
            The function  takes `data_batch`
            as an input and returns a dict of
            str -> NDArray. The resulting dict is used for pulling row_sparse
            parameters from the kvstore,
            where the str key is the name of the param,
        ...
        """
    assert model.binded and model.params_initialized
    ...
    pre_score_array, y_label_array = np.array([]), np.array([])

    for nbatch, eval_batch in enumerate(eval_data):
        if num_batch is not None and nbatch == num_batch:
            break
        model.prepare(eval_batch, sparse_row_id_fn=sparse_row_id_fn)
        model.forward(eval_batch, is_train=False)
        pred_score = model.get_outputs()[0].asnumpy()
        class_num = pred_score.shape[1]
        pre_score_array = np.append(pre_score_array,
                                    pred_score).reshape(-1, class_num)
        y_label = eval_batch.label[0].asnumpy()
        y_label_array = np.append(y_label_array, y_label)
        if isinstance(eval_batch, list):
            model.update_metric(
                eval_metric, [eb.label for eb in eval_batch], pre_sliced=True)
        else:
            model.update_metric(eval_metric, eval_batch.label)
    ...
    y_label_onehot = to_onehot(
        y_label_array, num_classes=pre_score_array.shape[1])
    c_evaluate = ClassificationEvaluate(y_label_onehot, pre_score_array)
    evaluate_result = dict()
    evaluate_result['precision'] = c_evaluate.get_precision()
    evaluate_result['recall'] = c_evaluate.get_recall()
    evaluate_result['f1'] = c_evaluate.get_f1()
    evaluate_result['mAP'] = c_evaluate.get_mAP()
    evaluate_result['PR_line'] = c_evaluate.get_pr_line()
    evaluate_result['ROC_line'] = c_evaluate.get_roc_line()

    return eval_metric.get_name_value(), evaluate_result
...
def train_fit(model,
    ...
    """Trains the module parameters.
        ...
        train_data : DataIter
            Train DataIter.
        eval_data : DataIter
            If not ``None``, will be used as validation set
            and the performance
            after each epoch will be evaluated.
        eval_metric : str or EvalMetric
            Defaults to 'accuracy'. The performance measure
            used to display during training.
            Other possible predefined metrics are:
            'ce' (CrossEntropy), 'f1', 'mae', 'mse', 'rmse', 'top_k_accuracy'.
        epoch_end_callback : function or list of functions
            Each callback will be called with
            the current `epoch`, `symbol`, `arg_params`
            and `aux_params`.
        ...
        optimizer_params : dict
            ...
            just to avoid pylint warning on dangerous
            default values.
        ...
        eval_batch_end_callback : function or list of function
            These will be called at the end of
            each mini-batch during evaluation.
        initializer : Initializer
            The initializer is called to
            initialize the module parameters when they are
            not already initialized.
        arg_params : dict
            Defaults to ``None``, if not ``None``,
            should be existing parameters from a trained
            model or loaded from a checkpoint (previously saved model).
            In this case,
            the value here will be used to initialize the module parameters,
            unless they
            are already initialized by the user
            via a call to `init_params` or `fit`.
            `arg_params` has a higher priority than `initializer`.
        aux_params : dict
            Defaults to ``None``. Similar to `arg_params`,
            except for auxiliary states.
        allow_missing : bool
            Defaults to ``False``. Indicates whether to
            allow missing parameters when `arg_params`
            and `aux_params` are not ``None``.
            If this is ``True``, then the missing parameters
            will be initialized via the `initializer`.
        force_rebind : bool
            Defaults to ``False``.
            Whether to force rebinding the executors if already bound.
        force_init : bool
            Defaults to ``False``.
            Indicates whether to force initialization even if the
            parameters are already initialized.
        begin_epoch : int
            Defaults to 0. Indicates the starting epoch. Usually,
            if resumed from a
        ...
        load_checkpoint(model_prefix, 3)
        >>> mod.fit(train_data=train_dataiter,
        eval_data=val_dataiter, optimizer='sgd',
    ...
    for name, val in res:
        model.logger.info('Epoch0 Validation-{}={}'.format(name, val))
    for name in e_res:
        model.logger.info('Epoch0 Validation-{}={}'.format(name, e_res[name]))
    ...
    for epoch in range(begin_epoch, num_epoch):
        ...
        while not end_of_batch:
            ...
            try:
                ...
                next_data_batch = next(data_iter)
                model.prepare(
                    next_data_batch, sparse_row_id_fn=sparse_row_id_fn)
        ...
        if eval_data:
            ...
            for name, val in res:
                model.logger.info('Epoch{} Validation-{}={}'.format(
                    epoch, name, val))
            for name in e_res:
                model.logger.info('Epoch{} Validation-{}={}'.format(
                    epoch, name, e_res[name]))
...
def fit(args, network, data_loader, **kwargs):
    ...
    if args.profile_server_suffix:
        mx.profiler.set_config(
            filename=args.profile_server_suffix,
            profile_all=True,
    ...
    if args.profile_worker_suffix:
        if kv.num_workers > 1:
            filename = 'rank' + str(kv.rank) + \
                       '_' + args.profile_worker_suffix
        else:
            filename = args.profile_worker_suffix
        mx.profiler.set_config(
            filename=filename, profile_all=True, profile_process='worker')
    ...
    if 'dist' in args.kv_store and 'async' not in args.kv_store:
        ...
        # each machine has same number of batches per epoch
        # if not, dist_sync can hang at the end
        # with one machine waiting for other machines
    ...
    if len(args.loss) > 0:
        ...
        if 'softmax_output' in network.list_outputs():
            ...
            logging.warning('The output is not softmax_output, \
                loss argument will be skipped!')
