...
def add_data_aug_args(parser):
    ...
    aug.add_argument(
        ...
        type=float,
        default=None,
        help='min value of aspect ratio, \
        whose value is either None or a positive value.')
    aug.add_argument(
        '--max-random-aspect-ratio',
        type=float,
        default=0,
        help='max value of aspect ratio. \
        If min_random_aspect_ratio is None, '
    ...
    aug.add_argument(
        ...
        type=float,
        default=1,
        help='max area to crop in random \
        resized crop, whose range is [0, 1]')
    aug.add_argument(
        '--min-random-area',
        type=float,
        default=1,
        help='min area to crop in random resized crop, \
        whose range is [0, 1]')
    aug.add_argument(
        '--min-crop-size',
        type=int,
...
class SyntheticDataIter(DataIter):
    ...
    def provide_label(self):
        return [
            mx.io.DataDesc('softmax_label', (self.batch_size, ), self.dtype)
        ]
...
def get_rec_iter(args, kv=None):
    image_shape = tuple([int(il) for il in args.image_shape.split(',')])
