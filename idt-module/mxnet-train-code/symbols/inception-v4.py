...
def Inception_stem(data, name=None):
    c = Conv(
        data, 32, kernel=(3, 3), stride=(2, 2), name='%s_conv1_3*3' % name)
    ...
    c1 = Conv(
        concat, 64, kernel=(1, 1), pad=(0, 0), name='%s_conv5_1*1' % name)
    c1 = Conv(c1, 96, kernel=(3, 3), name='%s_conv6_3*3' % name)

    c2 = Conv(
        concat, 64, kernel=(1, 1), pad=(0, 0), name='%s_conv7_1*1' % name)
...
def ReductionA(input, name=None):
    ...
    c3 = Conv(
        input, 192, kernel=(1, 1), pad=(0, 0), name='%s_conv2_1*1' % name)
...
def InceptionB(input, name=None):
    ...
    c2 = Conv(
        input, 384, kernel=(1, 1), pad=(0, 0), name='%s_conv2_1*1' % name)

    c3 = Conv(
        input, 192, kernel=(1, 1), pad=(0, 0), name='%s_conv3_1*1' % name)
    c3 = Conv(c3, 224, kernel=(1, 7), pad=(0, 3), name='%s_conv4_1*7' % name)
    # paper wrong
    c3 = Conv(c3, 256, kernel=(7, 1), pad=(3, 0), name='%s_conv5_1*7' % name)

    c4 = Conv(
        input, 192, kernel=(1, 1), pad=(0, 0), name='%s_conv6_1*1' % name)
...
def ReductionB(input, name=None):
    ...
    c2 = Conv(
        input, 192, kernel=(1, 1), pad=(0, 0), name='%s_conv1_1*1' % name)
    c2 = Conv(
        c2, 192, kernel=(3, 3), stride=(2, 2), name='%s_conv2_3*3' % name)

    c3 = Conv(
        input, 256, kernel=(1, 1), pad=(0, 0), name='%s_conv3_1*1' % name)
    c3 = Conv(c3, 256, kernel=(1, 7), pad=(0, 3), name='%s_conv4_1*7' % name)
    c3 = Conv(c3, 320, kernel=(7, 1), pad=(3, 0), name='%s_conv5_7*1' % name)
    c3 = Conv(
        c3, 320, kernel=(3, 3), stride=(2, 2), name='%s_conv6_3*3' % name)
...
def InceptionC(input, name=None):
    ...
    c2 = Conv(
        input, 256, kernel=(1, 1), pad=(0, 0), name='%s_conv2_1*1' % name)

    c3 = Conv(
        input, 384, kernel=(1, 1), pad=(0, 0), name='%s_conv3_1*1' % name)
    c3_1 = Conv(c3, 256, kernel=(1, 3), pad=(0, 1), name='%s_conv4_3*1' % name)
    c3_2 = Conv(c3, 256, kernel=(3, 1), pad=(1, 0), name='%s_conv5_1*3' % name)

    c4 = Conv(
        input, 384, kernel=(1, 1), pad=(0, 0), name='%s_conv6_1*1' % name)
    ...
    c4_2 = Conv(
        c4, 256, kernel=(1, 3), pad=(0, 1), name='%s_conv10_3*1' % name)
