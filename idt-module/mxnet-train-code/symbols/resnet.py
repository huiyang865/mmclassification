...
def resnet(units,
    ...
    for i in range(num_stages):
        body = residual_unit(
            body,
            filter_list[i + 1], (1 if i == 0 else 2, 1 if i == 0 else 2),
            False,
    ...
    prob1 = 0.9
    do1 = mx.sym.Dropout(data=relu1, name='do1', p=prob1)
    ...
    pool1 = mx.sym.Pooling(
        ...
        name='pool1')
    prob2 = 0.9
    do2 = mx.sym.Dropout(data=pool1, name='do2', p=prob2)

    fc1 = mx.sym.FullyConnected(data=do2, num_hidden=num_classes, name='fc1')
    if dtype == 'float16':
...
def get_symbol(num_classes,
    ...
    Original author Wei Wu
    """
    image_shape = [int(il) for il in image_shape.split(',')]
    (nchannel, height, width) = image_shape
    if height <= 28:
        num_stages = 3
