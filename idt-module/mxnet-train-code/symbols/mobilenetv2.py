...
def shortcut(data_in, data_residual, prefix):
    out = mx.sym.elemwise_add(
        data_in, data_residual, name='%s-shortcut' % prefix)
    return out


def mobilenet_unit(data,
                   num_filter=1,
                   kernel=(1, 1),
                   stride=(1, 1),
                   pad=(0, 0),
...
def inverted_residual_unit(data, num_in_filter, num_filter, ifshortcut, stride,
                           kernel, pad, expansion_factor, prefix):
...
class MobileNetV2(object):
    ...
    def build_network(self, class_num=1000, **configs):
        ...
        last_fm = mobilenet_unit(
            data=last_bottleneck_layer,
            num_filter=int(1280 *
                           self.multiplier) if self.multiplier > 1.0 else 1280,
            kernel=(1, 1),
            stride=(1, 1),
            pad=(0, 0),
            if_act=True,
        ...
        fc = mx.symbol.FullyConnected(
            data=flatten, num_hidden=class_num, name='fc')
    ...
    def __call__(self, class_num=1000, layer_out=None, **configs):
        ...
        if type(layer_out) is list or type(layer_out) is tuple:
            layers_out = [
                internals[layer_nm.strip() + '_output']
